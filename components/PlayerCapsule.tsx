...
export const PlayerCapsule: React.FC<PlayerCapsuleProps> = ({ onToast, onExpand, onNavigateToArtist, onNavigateToAlbum, className = '' }) => {
  ...
  return (
    <div
        className={`fixed bottom-8 left-1/2 -translate-x-1/2 z-[80] w-full max-w-4xl px-4 ${className}`}
        onClick={(e) => e.stopPropagation()}
    >
         {/* Volume Pop-up */}
         <div
            ref={volumePopupRef}
            className={`absolute right-0 bottom-full mb-4 bg-[#0a0a0a] border border-white/10 rounded-2xl p-4 flex flex-col items-center gap-4 ${isLinux ? '' : 'transition-all duration-300'} origin-bottom shadow-2xl z-[90] ${showVolume ? 'opacity-100 scale-100' : 'opacity-0 scale-90 pointer-events-none'}`}
         >
            <div
              ref={volumeRef}
         ...
         {/* Main Capsule */}
         <div
            className={`${isLinux ? 'bg-[#050505]' : 'bg-[#050505]/80 backdrop-blur-2xl'} border border-white/10 rounded-full p-2 pr-6 shadow-[0_30px_60px_rgba(0,0,0,0.8)] flex items-center gap-4 relative overflow-hidden group hover:bg-[#050505]/90 ${isLinux ? '' : 'transition-all duration-500'}`}
         >

...
export const FullScreenPlayer: React.FC<FullScreenPlayerProps> = ({ onCollapse, onToast, onNavigateToArtist }) => {
    ...
    return (
        <div className={`fixed inset-0 z-[200] flex flex-col overflow-hidden ${isLinux ? '' : 'animate-in slide-in-from-bottom duration-500'}`}>

            {/* Soft, colorful gradient base — lighter Apple Music inspired */}
            <div className={`absolute inset-0 ${isLinux ? 'bg-[#e8ddf0]' : 'bg-gradient-to-br from-[#e4d4f0] via-[#d9c4ec] to-[#cfd4f5]'}`} />

            {/* Vivid atmospheric orbs — reduced blur for performance */}
            {!isLinux && (
            ...
            <button
                onClick={onCollapse}
                className={`absolute top-6 left-6 z-50 p-3 rounded-full ${
                    isLinux ? 'bg-black/5' : 'bg-white/40 backdrop-blur-md'
                } hover:bg-black/10 text-neutral-500 hover:text-neutral-800 transition-colors border border-white/30`}
            >
            ...
            <div className="relative z-10 flex flex-1 min-h-0 pt-16 pb-4 px-8 gap-8">
                ...
                <div className="flex flex-col items-center w-[380px] flex-shrink-0 justify-center">
                    ...
                    <div className={`w-[300px] h-[300px] rounded-2xl overflow-hidden flex-shrink-0 shadow-2xl shadow-black/15 border ${isLinux ? 'border-black/5' : 'border-white/50'}`}>
                        <AlbumArt
                                ...
                                <>
                                    ...
                                    {lyrics.map((lyric, i) => (
                                        <p
                                            key={i}
                                            data-lyric-index={i}
                                            className={`text-2xl lg:text-4xl font-bold cursor-pointer hover:opacity-100 ${
                                                isLinux ? '' : 'transition-all duration-500'
                                            } ${
                                                i === activeLyricIndex
                                                    ? 'text-neutral-900 scale-100'
                                                    : `text-neutral-400/40 hover:text-neutral-600 ${isLinux ? '' : 'blur-[1px] scale-[0.97]'}`
                                            }`}
                                            onClick={() => {
            ...
            <div
                ref={volumePopupRef}
                className={`absolute bottom-44 left-[340px] z-50 ${
                    isLinux ? 'bg-white/90' : 'bg-white/60 backdrop-blur-xl'
                } border border-white/40 rounded-2xl p-4 flex flex-col items-center gap-3 shadow-xl ${
                    isLinux ? '' : 'transition-all duration-300'
                } origin-bottom ${showVolume ? 'opacity-100 scale-100' : 'opacity-0 scale-90 pointer-events-none'}`}
            >
                <div
                    ref={volumeSliderRef}

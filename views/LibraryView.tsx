                    ...
                    {arrangeAlbumsForGrid(groupedAlbums[year] || []).map((item: Album) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className={`absolute bottom-2 left-2 px-1.5 py-0.5 ${isLinux ? 'bg-black/80' : 'bg-black/60 backdrop-blur-sm'} rounded text-[9px] font-mono text-white/80 ${isLarge ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} transition-opacity`}>
                            {item.trackCount}
                          </div>
